...
* **Current Status**: A comprehensive set of baseline solvers exists in Python. Draft articles have been written in LaTeX. The robustness hypothesis **H1** (a covering-based **outer approximation** of orders that recasts CSLAP as a data-driven robust counterpart) was first taken end-to-end and ACCEPTED as conditional, then **re-validated this phase on an exact CPLEX cover + the clean κ-invariant `ISCF Data/` instance (CPLEX-revalidation phase, COMPLETE).** Outcome: **H1 is FALSIFIED at κ=2 on the clean exact instance** — a real, stable *price* of robustness (PoR>0) but a *negative* out-of-sample gain (G<0) under both 5-fold CV and a temporal split, both 95% CIs excluding zero, every fold agreeing, feasibility clean. Prop. 1's robust-counterpart identity is intact; the empirical conjecture H1 adds on top of it does not hold here. See `reports/7_scientific_report_iscf_cplex.md` and the updated `LaTeX_Articles_We_Have_Drafted/CSLAP_Robust_Covering_draft.tex`. See §7.
...
## 7. Current Research State (CPLEX-revalidation phase COMPLETE)

**The original plan lives in [`plan.md`](file:///c:/Users/ebelul/My%20Savoye%20project/CSLAP_Problem/plan.md); the phase it defines is now DONE. The authoritative result is `reports/7_scientific_report_iscf_cplex.md`.**

**Outcome (read first).** The exact CPLEX re-validation on the clean κ-invariant ISCF instance **FALSIFIES H1 at κ=2**: PoR(κ=2) is positive (CV +6.8% [4.8,8.8]; temporal +5.1% [2.0,8.2]) and the out-of-sample gain G(κ=2) is **negative** (CV −6.8% [−9.0,−4.7]; temporal −7.0% [−10.3,−3.8]), both CIs excluding zero, all 5 folds sign-consistent, feasibility clean (cap_broken=0). With κ-invariant capacity there is **no free lunch** (the first run's negative-PoR signature was the recalibration confound). Prop. 1 (robust-counterpart identity) is untouched — it concerns the training objective; the empirical conjecture H1 adds (future orders fall inside Û_κ often enough to win out-of-sample) does not hold here. **This supersedes the earlier "conditional" reading.** New code: `Baselines/iscf_adapter.py`, `Baselines/run_robustness_iscf.py`, `--backend cplex` (incremental raw-`cplex` master) in `pk_cover_construction.py`, κ-invariant flag in `pk_instance_robust.py`. Two venvs: cover=`Virtual_Environment_CPLEX_1`, placement(Hexaly `milp_synthetic.py`)=`Virtual_Environment_LocalSolver_3`.

**Open edges (future work, NOT this phase).** (i) Only κ=2 certified — the set-partition integer master is intractable-to-optimality for κ≥3 at scale, so no PoR/G curve / interior optimum on the clean instance. (ii) Exact cover scales only to a few hundred SKUs → ran on the iscf480 sub-instance, not full ISCF (4328) or BERNER (21k). (iii) Placement is Hexaly (metaheuristic), per user directive — exact placement MILP NOT used.

**Original "Where we are" (historical).** Hypothesis **H1** — enlarge each order to its covering pattern-closure, then optimize placement on the enlarged instance — was taken once through the pipeline; the first result was **conditional** (stable price, unstable gain). The phase below re-tested it.
